...
feature-oriented domain prototype with a working account service, a working journal service, validated journal and
posting models, journal input and view DTOs, a journal repository contract, shared error translation, and Rich-based
journal rendering. Storage-backed workflows, reporting, and operational CLI commands are not implemented yet.
...
Only the first stage is currently implemented as a live workflow. There is no trial balance pipeline or downstream
reporting layer, and no workflow converts journal entries into ledger postings yet.
...
- `PostingService` is commented out and references stale API concepts such as `chart.resolve()`, so it should be
  treated as a scaffold only.
...
- `tests/fakes/account_repo.py` and `tests/fakes/journal_repo.py` provide in-memory repository fakes for service tests.
...
- Current automated coverage is concentrated on domain models, shared validation helpers, shared error translation,
  `AccountService`, and `JournalService`.
...
- There are no `PostingService`, storage, reporting, or CLI workflow tests yet.
